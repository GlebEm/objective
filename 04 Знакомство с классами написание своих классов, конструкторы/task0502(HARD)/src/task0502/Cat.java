...
public class Cat {
    public int age;
    public int weight;
    public int strength; //2. Класс Cat должен содержать всего три поля: age, weight и strength. Поля должны быть публичные.

    public Cat() {  //1. Класс Cat должен содержать конструктор без параметров.
    }

    public boolean fight(Cat anotherCat) { //3. В классе Cat должен быть метод fight.
        int cat1count = 0;
        int cat2count = 0;
    ...
    public static void main(String[] args) {
        ...
        if (cat1.fight(cat2)) {
            System.out.println("Первый кот победил! Второй кот проиграл");
        } else if (!cat1.fight(cat2))
            System.out.println("Ничья или второй кот победил!");
    }
}
